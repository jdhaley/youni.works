export default {
	...
	Compiler: {
		type$: "Instance",
		constructObject: function(properties) {
			const sys = this.sys;
			properties[Symbol.status] = "[Constructing]";
			let type = properties[""];
			...
			let object = Object.create(type || null);
			sys.define(object, sys.symbols.interface, properties);
			return object;
		},
		extendObject: function(object, properties) {
			for (let name in properties) {
				if (name) sys.define(object, name, value[name]);
			}

			//this.compileProperties(value);

		},
		compileClass: function(object, properties) {
			throw new Error("unsupported.");
			
			

//			const componentName = object[sys.symbols.name];
//			const propertyName = componentName.substring(componentName.lastIndexOf("/") + 1);
//			const proper = propertyName.charAt(0);
//			if (proper == proper.toUpperCase() && proper != proper.toLowerCase()) {
//				sys.define(target, sys.symbols.type, propertyName);
//			}
...
//			}
		...
		compileValue: function(value) {
			switch (this.sys.statusOf(value)) {
				case "Object":
					let object = this.constructObject(value);
					value = object[this.sys.symbols.interface];
		...
		compileProperty: function(object, propertyName) {
			...
			//		Reflect.defineProperty(object, value.name, value);
					return;
				case "Object":
					object[propertyName] = this.constructObject(value);
					this.compileProperties(value[this.sys.symbols.interface])
					this.extendObject(value);
					return;
				case "Properties":
					this.compileProperties(value);
					return;
