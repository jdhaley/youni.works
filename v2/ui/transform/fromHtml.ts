...
type transform = (source: Node, target: Item[], level?: number) => void;

const methods: bundle<transform> = {
	strip(source: Node, target: Item[], level?: number) {
	},
	transform(source: Node, target: Item[], level?: number) {
		for (let node of source.childNodes) {
			let fn = tags[node.nodeName] || tags.default;
			fn(node, target, level || 0);
		}
	},
	list(source: Node, target: Item[], level: number) {
		return methods.transform(source, target, (level || 0) + 1);
	},
	section(source: Node, target: Item[]) {
		let item = {
			type$: "heading",
	...
	line(source: Node, target: Item[], level: number) {
		//HANDLE OFFICE
	...
	link(source: Element, target: Item[], level: number) {
...
function append(item: Item, text: string) {
	if (item) {
		if (!item.content) item.content = "";
		let ch = text.at(0);
		let needSpace = ch.toLowerCase() != ch.toUpperCase() || ch == "<";
		if (needSpace && !item.content.endsWith(" ")) text = " " + text;
		item.content += text;   
	}
}
